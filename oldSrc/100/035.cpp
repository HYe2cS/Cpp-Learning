...
int larray[2][200];     /*存放素数前二、三位数的临时数组所对应的数量计数
器*/
...
select[selecount++] = count - 1;
	...
	larray[0][lcount[0]++] = number[0][0] / 100;     /*临时数组的第一行存前二
位*/

	larray[1][lcount[1]++] = number[0][0] / 10;      /*临时数组的第二行存前三
位*/

	for (i = 1; i < count; i++)                /*将素数不重复的前二、三位存入临
时数组中*/
	...
	for (i1 = 0; i1 < selecount; i1++)                    /*在第一行允许的汇聚围
内穷举*/

	{
		...
		for (array[1][0] = 0; array[1][0] < count; array[1][0]++)    /*穷举第二
行*/

		{
			...
			if (!comp_num(2))
				...
				continue;         /*若每列的前两位的组成与素数相矛盾，则
试探下一个数*/

			for (array[2][0] = 0; array[2][0] < count; array[2][0]++)     /*穷举
第三行*/

			{
				...
				if (!comp_num(3))
					...
					continue;          /*若每列的前三位的组成与素数相矛盾
，则试探下一个数*/

				for (i4 = 0; i4 < selecount; i4++)     /*在最后一行允许的范围内
穷举*/

				{
					...
					for (flag = 1, i = 1; flag && i <= 4; i++)    /*判断每列是否可逆
素数*/

if (!find1(i))flag = 0;

					if (flag && find2())            /*判断对角线是否为可逆素
数*/
...
void copy_num(int i)        /*将array[i][0]指向的素数的各位数字复制到arr
ay[i]中*/
...
int comp_num(int n)           /*判断array中每列的前n位是否与可逆素数允许
的前n位矛盾*/

{
	static int ii;
	static int jj;         /*用内部静态变量保存前一次查找到的元素下标*
/
static int jj;

	static int jj;    /*ii:前一次查找前二位的下标，jj:前一次查找前三位的
下标*/
...
/
			...
			for(;*p>=0&&num<larray[n-2][*p];(*p)--);/*若前次查找到的元素
大，则向前找*/
