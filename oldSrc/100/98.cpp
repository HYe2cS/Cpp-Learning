...

{
	...
	i = 1;          /*正在处理的元素下标，表示前i-1个元素已符合要求，正在处
 理第i个元素*/
	...
	{
		while (not_finish && i <= NUM)      /*处理尚未结束且还没处理到第NUM个元
  素*/
   ...
   /
			...
			if (!flag)     /*若存在矛盾不满足要求，需要重新设置第i个元
素*/

			{
				if (a[i] == a[i - 1])        /*若a[i]的值已经经过一圈追上a[
i-1]的值*/
 ...
 */
				...
				else a[i] = a[i - 1] + 1;        /*否则元素的值为前一个元素的下
 一个值*/
		}
